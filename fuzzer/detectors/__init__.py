...
class DetectorExecutor:
    def __init__(self, source_map=None, function_signature_mapping={}, settings=None):
        ...
        self.raise_on_error_found = settings.RUN_UNTIL_FIRST_BUG
    ...
    def run_detectors(self, previous_instruction, current_instruction, errors, tainted_record, individual, mfe, previous_branch, transaction_index):
        if self.settings.DETECT_ARBITRARY_MEM:
        ...
        if self.settings.DETECT_ASSERTION_FAIL:
        ...
        if self.settings.DETECT_INTEGER_OVERFLOW:
        ...
        if self.settings.DETECT_REENTRANCY:
        ...
        if self.settings.DETECT_TRANSACTION_ORDER_DEPENDENCY:
        ...
        if self.settings.DETECT_BLOCK_DEPENDENCY:
        ...
        if self.settings.DETECT_UNHANDLED_EXCEPTION:
            pc = self.unhandled_exception_detector.detect_unhandled_exception(previous_instruction, current_instruction, tainted_record)
            if pc and self.add_error(errors, pc, "Unhandled Exception", individual, mfe, self.unhandled_exception_detector, self.source_map):
        ...
        if self.settings.DETECT_UNSAFE_DELEGATECALL:
        ...
        if self.settings.DETECT_LEAKING_ETHER:
        ...
        if self.settings.DETECT_LOCKING_ETHER:
        ...
        if self.settings.DETECT_SELFDESTRUCT:
